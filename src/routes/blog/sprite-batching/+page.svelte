...
<div class="page-wrapper">
	...
	<section class="content-section">
		...
		<p>The second factor is the <b>texture unit</b>. When a GPU draws a texture onto the screen, it must sample that texture from one of its active texture units. Most modern GPUs only have 16 or 32 active texture units to bind at a time, but since each bound texture can be very large, it’s standard to include many individual textures together in a <b>texture atlas</b>, and provide the shader with specific image coordinates to get the required texture. This is known as <b>sampling</b>.</p>
		<p>By grouping up a large number of draw calls to the same atlas into a single instruction, performance can easily be increased 256-fold, if not more. This is the power of sprite batching.</p>
		...
		<p>But what does this say more broadly? Sprite batching is a prime example of designing for computational efficiency, where code is optimised for time or space complexity. Even in a simple program like Isola, for which modern hardware is sufficient enough to run the game without optimisations, <b>coding with efficiency in mind</b> builds a program that is robust, scalable, and futureproof, and trains a developer who is passionate, detail-oriented, and methodical.</p>
		<p>In some sense, <b>game development</b> has, since its very inception, tested the upper limits of computing. While domains such as web development can get away with heavy frameworks and an overreliance on client-side rendering, the top titles in gaming simply cannot afford to take computational liberties; every frame counts.</p>
